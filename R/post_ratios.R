...
posting_ratios <- function(data){
  ...
  print(post_data$count / sum(post_data$count))
  post_data$fraction = post_data$count / sum(post_data$count)
  
  post_data$percentage = post_data$count / sum(post_data$count) * 100
  post_data$ymax = cumsum(post_data$fraction)
  post_data$ymin = c(0, head(post_data$ymax, n=-1))
